...
public class FourWork {
    ...
    public static void player1Move() {
        ...
        do {
            ...
            x = Integer.parseInt(input.next()) - 1;
            ...
            for (int i = 0; i < FIELD_SIZE; i++) {
                ...
                if (k < FIELD_SIZE) {
                    if (gameField[i][k] == playerDot) { // проверяем главную диагональ от центральной оси вправо на возможную победу
                        diagMain++;
                    } else if (gameField[i][k] != playerDot) {
                        diagMain = 0;
                    }
                }
            ...
            for (int i = 0; i < FIELD_SIZE; i++) {
                ...
                if (k < FIELD_SIZE) {
                    if (gameField[k][i] == playerDot) { // проверяем главную диагональ от центральной оси вниз на возможную победу
                        diagMain++;
                    } else if (gameField[k][i] != playerDot) {
                        diagMain = 0;
                    }
                }
                if (diagMain >= DOTS_TO_WIN) {
        ...
        for (int i = 0; i < FIELD_SIZE; i++) {
            ...
            if (k >= 0 && l < FIELD_SIZE) {
                if (gameField[l][k] == playerDot) { // проверяем побочную диагональ от центральной оси вниз на возможную победу
                    diagReverse++;
                } else if (gameField[l][k] != playerDot) {
                    diagReverse = 0;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("Цель игры - заполнить линию подряд по вертикали, горизонтали или диагонали из %d Ваш%s символ%s.\n", DOTS_TO_WIN, "их", "ов");
